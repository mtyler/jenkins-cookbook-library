...
## curl -v -u $ADMIN_USR:$ADMIN_PWD -d '{"accessToken": "$GITHUB_TOKEN"}' -H "Content-Type:application/json" -XPUT http://localhost:8080/jenkins/blue/rest/organizations/jenkins/scm/github/validate
      ...
      java -jar ./jenkins-cli.jar -auth $ADMIN_USR:$ADMIN_PWD -s $JENKINS_URL create-credentials-domain-by-xml user::user::$ADMIN_USR < $WKDIR$BUILD_CONTEXT/blueocean-github-domain.xml
      if [ $? -eq 0 ]; then echo "Domain created!"; fi
      java -jar ./jenkins-cli.jar -auth $ADMIN_USR:$ADMIN_PWD -s $JENKINS_URL create-credentials-by-xml user::user::$ADMIN_USR blueocean-github-domain < $WKDIR$BUILD_CONTEXT/github-credentials.xml
